...
sys_exofork(void)
{
	...
	int alloc_result = env_alloc(&child_env,curenv->env_id);
...
sys_env_set_status(envid_t envid, int status)
{
	...
	struct Env* e;
	int eid_result=envid2env(envid,&e,1);
	if(eid_result<0) return eid_result;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	||(uintptr_t) dstva >= UTOP|| PGOFF(dstva) != 0) return -E_INVAL;
	...
	if(page_insert(dstenv->env_pgdir,src_page,dstva,perm)<0) return -E_NO_MEM;
